...
func CreateOrUpdateProduct(userProduct model.UserProduct, userProductCollection *mongo.Collection) error {
...
func GetAllProductsFromDB(userProductCollection *mongo.Collection) ([]model.UserProduct, error) {
	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
	defer cancel()
...
func GetProductByIDFromDB(id string, userProductCollection *mongo.Collection) (model.Product, error) {
	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
	defer cancel()
...
func UpdateProductInDB(userId, productId string, updateData map[string]interface{}, userProductCollection *mongo.Collection) error {
...
func DeleteProductInDB(userId, productId string, userProductCollection *mongo.Collection) error {
	filter := bson.M{
		"UserId": userId,
		"Products": bson.M{
			"$elemMatch": bson.M{
